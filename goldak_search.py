        ...
        for test_i in range(80):
            ...
            goldak_range = torch.cat(
                [Ec.parameters_values[1:2, :].to(Ec.device),  # range for the thermal conductivity factor
                 ...
                 torch.Tensor([[0.2, 0.8]]).to(Ec.device)], axis=0).to(Ec.device)  # range for abasorptivty
            ...
            #goldak_ini[2] = np.random.uniform(0.02,0.05)#

            #goldak_ini[-1] = np.random.uniform(0.5, 0.8)
            ...
            optimizer = optim.LBFGS([goldak_ini], lr=0.1, max_iter=2000, max_eval=5000,history_size=100,line_search_fn="strong_wolfe", tolerance_change=1.0*np.finfo(float).eps)
            #lr=0.06, max_iter=50, history_size=100)#, line_search_fn="strong_wolfe", tolerance_change=1.0*np.finfo(float).eps)
            ...
            for epoch in range(3):
                def closure():
                    ...
                    # predict
                    goldak_new = goldak_ini # torch.Tensor([0.003,0.025,0.03,0.025,0.025,0.26]).to(Ec.device)#
                    goldak = goldak_new  * torch.full(
                        size=(grid_mesh.shape[0], goldak_range.shape[0]), fill_value=1.0).to(Ec.device)

                    absorptivity = goldak[:, -1:]
                    inputs = torch.cat([grid_t, grid_mesh, fc, goldak[:, 0:1], lp*absorptivity,  ss, goldak[:, 1:-1]],
                    ...
                    # loss
                    goldak_loss = torch.sum(torch.relu_(((goldak_ini-goldak_mean)**2)**0.5-goldak_halrang)) + \
                                  torch.sum((-goldak_ini+((goldak_ini)**2)**0.5)[1:])
            ...
            # report
            print(r"The optimization loss is %f",running_loss[-1],"Goldak parameters: ",goldak_ini.cpu().detach().numpy(),"free channel: ",fc[0,0].item())

            if running_loss[-1] < 20:
                goldak_rec.append(np.append(goldak_ini.cpu().detach().numpy(),fc[0,0].item()))#np.append(,a_ed.item()))
