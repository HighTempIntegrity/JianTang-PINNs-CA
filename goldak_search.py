...
def goldak_search(Ec,model, t, laser_power, scan_speed, mp_d_exp,mp_w_exp,Tm,absorptivity):
        ...
        for test_i in range(80):
            ...
            goldak_range = torch.cat(
                [Ec.parameters_values[1:2, :].to(Ec.device),  # range for the thermal conductivity factor
                 ...
                 torch.Tensor([[1.0,9.0]]).to(Ec.device),torch.Tensor([[-1.,0.8]]).to(Ec.device)],axis=0).to(Ec.device) # absorptivty = sigmoid(a * P/V + b)
            ...
            #goldak_ini[2] = np.random.uniform(0.02,0.1)#
            #goldak_ini[3] = np.random.uniform(0.05,0.15)
            #goldak_ini[4] = np.random.uniform(0.1,0.25)
            #goldak_ini[1] = np.random.uniform(0.15,0.25)
            ...
            optimizer = optim.LBFGS([goldak_ini], lr=0.5, max_iter=2000, max_eval=5000,history_size=100,line_search_fn="strong_wolfe", tolerance_change=1.0*np.finfo(float).eps)#, lr=0.06, max_iter=50, history_size=100)#, line_search_fn="strong_wolfe", tolerance_change=1.0*np.finfo(float).eps)
            ...
            for epoch in range(3):
                def closure():
                    ...
                    absorptivity = torch.sigmoid(goldak[:, -2:-1] * (lp * 1000) / ((ss * 1000)) + goldak[:,-1:])

                    inputs = torch.cat([grid_t, grid_mesh, fc, goldak[:, 0:1], lp*absorptivity,  ss, goldak[:, 1:5]],
                    ...
                    # loss
                    goldak_loss = torch.sum(torch.relu_(((goldak_ini-goldak_mean)**2)**0.5-goldak_halrang)[:5]) + \
                                  torch.sum((-goldak_ini+((goldak_ini)**2)**0.5)[1:-1])
            ...
            print(r"The optimization loss is %f",running_loss[-1],"Goldak parameters: ",goldak_ini,"free channel: ",fc[0,0].item(),
                  "Absorptivity: ",torch.unique(torch.sigmoid(goldak_ini[-2:-1] * (lp*1000)/((ss*1000)) + goldak_ini[-1:])).cpu().detach().numpy())
            ...
            if running_loss[-1] < 20:
                ...
                def melt_pool_measure(goldak_ini,grid_t, grid_mesh, lp, ss):
                    ...
                    inputs = torch.cat(
                        [grid_t, grid_mesh, fc, goldak[:, 0:1], lp * absorptivity, ss, goldak[:, 1:5]],
                        axis=1).type(torch.FloatTensor)
        ...
        goldak_a = goldak[:, 1:2] * (lp * 1000) / ((ss * 1000)) + goldak[:, -1:]
        inputs = torch.cat([grid_t, grid_mesh, fc, goldak[:, 0:1], lp * absorptivity, ss, goldak_a, goldak[:, 2:5]],
                           axis=1).type(torch.FloatTensor)
...
if __name__ == "__main__":
    ...
    model_path = 'network_20230715_153014/'#network20230328_230950/'#network20230405_140131/'#network20230331_212502/'#network20230405_115832/'#network20230403_142216/'#network20230331_212502/'#network20230328_230950/'##network20230311_140559/'#network20230306_190851/'#network20221227_152117/'#'
